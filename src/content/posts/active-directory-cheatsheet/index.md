...
updated: 2026-06-07
description: Cheatsheet for Active Directory privilege escalation during CTFs and Exams.
image: active-directory.png
tags:
  - cheatsheet
  - windows
  - privesc
...
- **Enumerate Domain Users:**
  ...
  #Get users with descriptions (look for stored passwords)
  Get-DomainUser -Properties description | Where-Object {$_.description -ne $null} | Select-Object samaccountname, description

  #Will return specific properties of a specific user
  Get-DomainUser -Identity [username] -Properties DisplayName, MemberOf | Format-List
  
  #Get users with "admin" in the name
  Get-DomainUser -Identity *admin* | Select-Object samaccountname, memberof
  
  #Get properties for a specific user
  Get-DomainUser -Identity <USERNAME> | Select-Object *
...
- **Enum Domain Computers:**
  ...
  ```powershell
	#Get all computer names (hostnames)
...
- **Enum Groups and Group Members:**
  ...
  ```powershell
  #Get all group names
  Get-DomainGroup | Select-Object -ExpandProperty samaccountname
  
  Get members of "Domain Admins"
  Get-DomainGroupMember -Identity "Domain Admins" | Select-Object -ExpandProperty membername
...
- **Enumerate Shares:**
  ...
  ```
  - Then, check [[AD SMB SHARES ENUMERATION]]
...
- **Enum ACLs:**
  ...
  Get-ObjectAcl -Identity <AccountName>
  ...
  Get-DomainObjectAcl -Identity * -ResolveGUIDs | ? { $_.SecurityIdentifier -match (Get-DomainUser -Identity <USERNAME>).sid }
  
  #Check for `GenericAll` (Full Control) rights
...
- **Enum Domain Trust:**
  ...
  #Get Domain Trusts (clean mapping)
  Get-DomainTrust | Select-Object SourceDomain, TargetDomain, TrustType, TrustDirection
  
  #Get Password Policy (min length, lockout)
...
- **User Hunting:**
  ...
  # Lists all users who are Domain Admins
  Get-DomainGroupMember -Identity "Domain Admins" -Recurse

  # **Protected Users.** Finds users considered "high privilege" by Active Directory (protected by AdminSDHolder).
...
- [fRunasCs](https://github.com/antonioCoco/RunasCs) Csharp and open version of windows builtin runas.exe
...
- PowerView:
  ...
  #Get potential Kerberoasting targets (users with SPNs)
